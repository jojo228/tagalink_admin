...
$sel = $car->query("select * from tbl_book where post_id=".$uid." and id=".$book_id."")->fetch_assoc();
   ...
   $paymentinfo = $car->query("select * from tbl_payment_list where id=".$sel['p_method_id']."")->fetch_assoc();
		...
		$userdata = $car->query("select name,mobile,ccode,profile_pic from tbl_user where id=".$sel['uid']."")->fetch_assoc();
		$pol['customer_name'] = $userdata['name'];
		$pol['customer_contact'] = $userdata['ccode'].$userdata['mobile'];
		$pol['customer_img'] = $userdata['profile_pic'];
		...
		$pol['pick_otp'] = $sel['pick_otp'];
		$pol['drop_otp'] = $sel['drop_otp'];

...
if($status == 'Booked')
{
$sel = $car->query("select * from tbl_book where uid=".$uid." and book_status!='Cancelled' and book_status!='Completed' order by id desc");
}
else 
{
$sel = $car->query("select * from tbl_book where uid=".$uid." and (book_status='Cancelled' or book_status='Completed') order by id desc");	
}
